...
class s {
  ...
  async authenticate() {
    if (!this.client) {
      throw new t("SiweFlow has no client instance");
    }
    try {
      let {
        message: e,
  ...
  async sign() {
    ...
    if (!this.preparedMessage) {
      throw new t("Could not prepare SIWE message");
    }
  ...
  async buildMessage() {
    ...
    let i = this.wallet.chainId.replace("eip155:", "");
    this.nonce ||= await this.getNonceOnce.execute();
    this.preparedMessage = n({
      address: e,
      chainId: i,
      nonce: this.nonce
    });
  ...
  constructor(t, i, n, s = false) {
    ...
    this.captchaToken = n;
    this.client = i;
    this._meta.disableSignup = s;
  }
}
export { s as SiweFlow };
